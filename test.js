...
function check_match() {
  ...
  for (let i = 0; i < childes.length; i++) {
    for (
      ...
      //만약 현재요소의 클래스에 seized가 있으면서 움직이는 요소가 아니라면
      if (
        el.classList.value.indexOf("seized") > 0 &&
        el.classList.value.indexOf("moving") <= 0
      ) {
        ...
        let els = {
          0: childes[i - 1]
            ...
            : null,
          1: childes[i].querySelectorAll("li > ul > li")[j - 1],
        ...
        console.log(el, " 현재 요소의 리스트 ", list);
        ...
        if (list.length >= 2) {
          ...
          el.classList =
            nextClassLevel(nowClassList.value.replace("seized", "")) +
            " seized";

          check_match();
          return;
        }
      } else if (el.classList.value.indexOf("seized") > 0) {
        //만약 아래 빈공간 있으면 아래로 내려야함
        // 내 아래요소
        let bottom = childes[i + 1]
          ? childes[i + 1].querySelectorAll("li > ul > li")[j]
          : null;
        ...
        console.log(
          "내 아래 요소",
          bottom,
          bottom ? bottom.classList.value : ""
        );

        if (bottom && bottom.classList.value == "") {
          console.log("바닥에 아무것도 없음");

          bottom.classList = el.classList;
